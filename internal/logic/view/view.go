...
func (s *sView) GetBreadCrumb(ctx context.Context, in *model.ViewGetBreadCrumbInput) []model.ViewBreadCrumb {
	...
	if in.ContentType != "" {
		uriPrefix = "/"
		topMenuItem, _ := service.Menu().GetTopMenuByUrl(ctx, uriPrefix)
		if topMenuItem != nil {
	...
	if uriPrefix != "" && in.CategoryId > 0 {
		category, _ := service.Category().GetItem(ctx, in.CategoryId)
		if category != nil {
			breadcrumb = append(breadcrumb, model.ViewBreadCrumb{
				Name: category.Name,
...
func (s *sView) RenderTpl(ctx context.Context, tpl string, data ...model.View) {
	...
	viewData["MainDomain"] = g.Cfg().MustGet(ctx, `setting.domain`).String()
	// 内置对象
	...
	if notice, _ := service.Session().GetNotice(ctx); notice != nil {
		_ = service.Session().RemoveNotice(ctx)
